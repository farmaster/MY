...
[task_local]
#jd免费水果
01 07,13,18 * * * jd_fruit.js, tag=东东农场3, img-url=https://raw.githubusercontent.com/58xinian/icon/master/jdnc.png, enabled=true
...
let lnStartAcc=Math.ceil(cookiesArr.length/3);
  ...
  console.log(`本次执行第${lnStartAcc+1}-${lnTotalAcc}个账号\n`);
...
function readShareCode() {
  return new Promise(async resolve => {
    $.get({url: `http://transfer.nz.lu/farm`, timeout: 1}, (err, resp, data) => {
      try {
        if (err) {
       //   console.log(JSON.stringify(err))
        console.log(`助力池网址错误！`)
